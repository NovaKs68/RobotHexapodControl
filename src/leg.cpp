...
std::vector<int> logIdServoErr(bool actionServos[3], int servoIdCoxa, int servoIdFemur, int servoIdTibia) {
    
    std::vector<int> servosIdErr;

    if (actionServos[0])
        servosIdErr.push_back(servoIdCoxa);
    
    if (actionServos[1])
        servosIdErr.push_back(servoIdFemur);
    
    if (actionServos[2])
        servosIdErr.push_back(servoIdTibia);
...
std::vector<int> Leg::posReadyToWalkAngleTime(double angleCoxa, double angleFemur, double angleTibia, double timeCoxa, double timeFemur, double timeTibia)
{
    Board &board = Board::Instance();
    for (int i = 1; i <= 2000; i++)
    {
        if (!board.getAction(m_servoIdCoxa) && !board.getAction(m_servoIdFemur) && !board.getAction(m_servoIdTibia))
        {
            ...
            std::vector<int> actionFinishedSucess = {0};
            return actionFinishedSucess;
        }
    ...
    bool actionServos[3] = {board.getAction(m_servoIdCoxa), board.getAction(m_servoIdFemur), board.getAction(m_servoIdTibia)};

    return logIdServoErr(actionServos, m_servoIdCoxa, m_servoIdFemur, m_servoIdTibia);
}
